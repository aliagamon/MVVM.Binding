...
namespace MVVM.Binding.EventSystems
{
    /// <summary>
    /// <para>Receives events from the EventSystem and calls regostered VM methods.</para>
    /// </summary>
    [AddComponentMenu("MVVM/Binding/Event Trigger")]
        ...
        IDataBindingBaseProvider
    {
        ...
        //}
        [Serializable]
        public class TriggerEvent : ISerializationCallbackReceiver, IDataBindingBaseProvider
        {
            [SerializeField]
            ...
            private MethodInfo GetValidMethodInfo(object obj, string funcName, Type[] argumentTypes)
            {
                ...
                for (System.Type type = obj.GetType(); type != typeof(object) && type != null; type = type.BaseType)
                {
                    ...
                    var isReactiveCommand = command.PropertyType.GetInterfaces().Any(i => i.IsGenericType &&
                                                                                          new [] {typeof(IReactiveCommand<>), typeof(IAsyncReactiveCommand<>)}
                                                                                              .Any(t => t.IsAssignableFrom(i.GetGenericTypeDefinition())));
            ...
            private class TriggerCall
            {
                ...
                public ViewModelBase Target => _view?.ViewModel ?? ViewModelProvider.Instance.GetViewModelBehaviour(_vmName);
                ...
                public bool IsValid()
                {
                    ...
                    return ViewModelProvider.IsViewModelTypeNameValid(_vmName) && !string.IsNullOrEmpty(_methodName);
                }
                ...
                public class CallBinding : IDataBindingBase
                {
                    private readonly Action _onRebind;
            ...
            public IEnumerable<IDataBindingBase> Bindings => _calls.Select(c => new TriggerCall.CallBinding(c, SetCallsDirty));
        }
        ...
        public IEnumerable<IDataBindingBase> Bindings => this.m_Delegates.SelectMany(d => d.callback.Bindings);
    }
}
